...
include 'img.php';
$query2='SELECT A.branch,B.id,B.name,B.email FROM `branch` as A inner join `employee` as B where A.id=B.branch';
...
include 'alert.php';
?>
<h1 class="h11">View Employees</h1>
<br>
<?php
...
<tr>
  ...
  <td class='ti'>".$row['id']."</td>
  <td class='ts'>".$row['name']."</td>
  <td class='ts'>".$row['email']."</td>
  <td class='ts'>".$row['branch']."</td>
  <td class='tu'><a data-bs-toggle='modal' data-bs-target='#".$row['name']."' href='#'>UPDATE</a></td>
  <div class='modal fade' id='".$row['name']."' tabindex='-1' aria-labelledby='".$row['name']."Label' aria-hidden='true'>
<div class='modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable'>
<div class='modal-content'>
  <div class='modal-header'>
    <h5 class='modal-title' id='".$row['name']."Label'>".$row['branch'].'/'.$row['name']."</h5>
    <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
  </div>
  <div class='modal-body'>";?>

<form action="eupdate.php" method="POST" class="form" enctype="multipart/form-data">
        <br>
       
    <input type="hidden" name="id" value="<?php echo $row['id']?>" class="bn" required readonly>
    <div class="container">
    <div class="row">
<div class="col-sm-6"> 
    ...
    <input type="text" name="ename" value="<?php echo $row['name']?>" placeholder="Employee Name" class="bn" required readonly> 
</div>
<div class="col-sm-6">
    <label for="">Email</label>
        <br><br>
    <input type="email" name="email" value="<?php echo $row['email']?>" placeholder="Email" class="bn" pattern="[a-z1-9]+@ecEmployee.com" title="ecEmployee.com missing after @" required autocomplete="off">
    </div>
    </div>
    </div>
...
<div class="col-sm-6"> 
    ...
    <label for="">Password</label>
        <br><br>
    <input type="text" name="pass" id="myInput" placeholder="Password" class="bn" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
    title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
</div>
<div class="col-sm-6"> 
    <label for="">Re_Password</label>
        <br><br>
    <input type="text" name="rpass" id="myInput1" placeholder="Confirm Password" class="bn" required>
    </div>
    </div>
    </div>
...
<br>
   ...
   $result = mysqli_query($connect, $query);
    while($row1=mysqli_fetch_assoc($result)){
        echo"<option value='$row1[id]'>".$row1['branch']."/".$row1['name']."</option>";
       }
   ?>
   </select>
...
<div class='modal-footer'>
  ...
  <input type='submit' value='Save changes' name='sub' class='btn btn-primary'>
</div>
</div>
</div>
</div>
</form>
<td class='td'><a href='bdel.php? id=$row[id]'>DELETE</a></td>
</tr>";

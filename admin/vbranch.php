...
include 'img.php';
$query2='SELECT A.id,A.branch,B.name FROM `branch` as A inner join cities_pk as B where A.city=B.id';
...
while($row=mysqli_fetch_assoc($con2))
{
    ...
    <tr>
      ...
      <td class='tu'><a data-bs-toggle='modal' data-bs-target='#".$row['branch'].$row['id']."' href='#'>UPDATE</a></td>
      <div class='modal fade' id='".$row['branch'].$row['id']."' tabindex='-1' aria-labelledby='".$row['branch'].$row['id']."Label' aria-hidden='true'>
  <div class='modal-dialog modal-dialog-centered modal-dialog-scrollable'>
    <div class='modal-content'>
      <div class='modal-header'>
        <h5 class='modal-title' id='".$row['branch'].$row['id']."Label'>".$row['branch'].'/'.$row['name']."</h5>
        <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
      </div>
      <div class='modal-body'>
      <form action='bupdate.php' method='POST' class='form form1'>
      "?>
      
    <br>
        <label for="">Branch_ID</label>
        <br><br>
    <input type="text" name="id" value="<?php echo $row['id']?>" class="bn" required readonly>
    <br><br>
        <br>
        <label for="">Branch_Name</label>
        <br><br>
    <input type="text" name="bname" value="<?php echo $row['branch']?>" class="bn" pattern="[A-Za-z0-9]+(_[A-Za-z0-9]+)*" title="Only letters, numbers, and underscores are allowed. Spaces at the beginning or end are not allowed." required>
    <br><br>
...
<select name="city" class="select">
    ...
    <?php
    $query3='SELECT * FROM `cities_pk`';
    $con=mysqli_query($connect,$query3);
    while($fetch=mysqli_fetch_assoc($con)){
       echo "<option class='opt1' value='$fetch[id]'>".$fetch['name']."</option>";
    }
    ?>
</select>
<br>

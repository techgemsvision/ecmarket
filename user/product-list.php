...
if(isset($_POST['submit'])){
    ...
    $query_search="SELECT * FROM `product` WHERE p_type=$product and (`keyword` like '$search%' or `keyword` like '%$search%' or `p_name` like '%$search') LIMIT 4";
    $conn1=mysqli_query($connect,$query_search);
    $query_search2="SELECT * FROM `product` WHERE p_type=$product and (`keyword` like '$search%' or `keyword` like '%$search%' or `p_name` like '%$search') LIMIT 9999999999999 OFFSET 4";
    $conn4=mysqli_query($connect,$query_search2);
}

$expensive="SELECT * FROM `product` WHERE p_type=1 or p_type=2 or p_type=3 and (`status`='on') GROUP by p_price DESC LIMIT 6";
$conn2=mysqli_query($connect,$expensive);
$expensive1="SELECT * FROM `product` WHERE p_type=1 or p_type=2 or p_type=3 and (`status`='on') GROUP by p_price DESC LIMIT 6,8";
...
?>
        ...
        <main class="main">
            ...
            <div class="page-content">
              ...
              while($fetch1=mysqli_fetch_assoc($conn1))
              {
                ...
                if($fetch1['stock']<=20){
                    ...
                    <span class="product-label label-new">upto '.$fetch1['discount'].' % off </span>';
                }
                
                else if($fetch1['discount']>=45){
                              echo'  <span class="product-label label-top">Best Deal </span>
                                <span class="product-label label-new">upto '.$fetch1['discount'].' % off </span>';
                }
                
                else{
                    echo '<span class="product-label label-new">'.$fetch1['discount'].' % off </span>';
                }
                                  ?>  
                                <?php echo '<div>
                        <img src="../admin/f_image/'.$fetch1['p_image'].'" alt="Product image" class="product-image img2">
                    </div>
     ...
     <input type="hidden" name="p_name" value="'.$fetch1['p_name'].'">
     <input type="hidden" name="p_image" value="'.$fetch1['p_image'].'">
     <input type="hidden" name="discount" value="'.$fetch1['discount'].'">
     <input type="hidden" name="original_price" value="'.$fetch1['original_price'].'">
     <input type="hidden" name="stock" value="'.$fetch1['stock'].'">
     <input type="hidden" name="p_price" value="'.$fetch1['p_price'].'">
     <div class="text-left">';?>
     <?php 
     if($fetch1['stock']>20){
          ...
          echo'</div>
            ...
            <div class="col-lg-6">
                <div class="product-body product-action-inner">
                    <h3 class="product-title">'.$fetch1['p_name'].'</h3>
<hr>
                    <div class="product-content">
                        <p><b class="text-dark">Description : </b>'.$fetch1['description'].'</p>
                    </div>
                    <br>
                    <div class="product-price"> <b class="text-dark">Original-Price : </b><span>'
                       .$fetch1['original_price']. 
                    '</span> $</div>
                    <br>
                ...
                echo '<h1 class="h1h1">Invalid Name! please search again</h1>';
            }
        }
        else
        {
    ...
    <div class="col-sm-6 col-xl-12 col-xxl-6">
        <div class="banner banner-overlay bolay">
            ...
            <div class="banner-content">
                ...
                <h3 class="banner-title text-white"><a href="#">Cloths upto <br><span>Up To 5% Off</span></a></h3><!-- End .banner-title -->
...
if($product==2){
    ...
    <div class="col-sm-6 col-xl-12 col-xxl-6">
        <div class="banner banner-overlay bolay">
            <a href="jewelry.php">
                <img src="assets/images/demos/demo-14/banners/100.jpg" alt="Banner img desc">
            </a>
    
            <div class="banner-content">
                <h4 class="banner-subtitle text-white"><a href="#">Best Deals</a></h4><!-- End .banner-subtitle -->
                <h3 class="banner-title text-white"><a href="#">Bangle<br><span>Up To 30% Off</span></a></h3><!-- End .banner-title -->
                <a href="jewelry.php" class="banner-link">Discover Now <i class="icon-long-arrow-right"></i></a>
            </div>
        </div>
    </div>
...
?>
        ...
        <?php
if(isset($conn4)){
    if(mysqli_num_rows($conn4)>0){
                while($expen=mysqli_fetch_assoc($conn4)){
             echo    

...
if(isset($_POST['sub'])){
    $track=$_POST['track'];
    $query="SELECT * FROM `order_items` WHERE `tracking_id`=$track and `branch`=$b";
    $conn=mysqli_query($connect,$query);
    $query12="SELECT * FROM `order_details` WHERE `tracking_id`=$track and `branch`=$b";
    $conn12=mysqli_query($connect,$query12);
    $rows=mysqli_num_rows($conn);
    $fetch2=mysqli_fetch_assoc($conn);
    $fetch12=mysqli_fetch_assoc($conn12);
}
            ...
            <nav class="navbar">
                ...
                <div class="navbar-nav w-100">
                   ...
                   <br>
                    ...
                    <a href="manage.php" class="nav-item active nav-link"><i class="fa fa-list-ol me-2"></i>ManageOrders</a>
...
<div class="text-center">
    ...
    <label for="">Tracking ID</label>
        <br><br>
    <input type="number" name="track" placeholder="Insert Tracking Id" class="bn bnn1" required>
    </div>

</div>
</div>
   <br><br><br>
<input type="submit" value="Search Items" name="sub" class="btn1">
<br></br>
    </form>
...
if (isset($conn)) {
    if ($rows > 0) {
        ...
        if ($fetch2['status'] == 'pending') {
            echo "<div class='text-white text-left'> <b>Order discount : </b> ".$fetch12['coupan discount']." % </div><br>"?>
            ...
            <table class='table'>
                <thead>
                    <tr>
                        ...
                        <th>Price</th>
                        <th>Qtn</th>
                        <th>Delete Quantity / Row</th>
                    </tr>
                </thead>";
                mysqli_data_seek($conn, 0); 
            while ($item = mysqli_fetch_assoc($conn)) {
                echo "<tbody class='me-3'>
                <tr>
                    <td class='ts'>".$item['item_name']."</td>
                    <td class='ts'>".$item['price']."</td>
                    <td class='ts'>".$item['qtn']."</td>
                    <td class='ts' colspan='2'>
                        <form action='deleteitem.php?id=".$item['id']."&item_name=".$item['item_name']."&qtn=".$item['qtn']."&price=".$item['price']."&tracking_id=".$item['tracking_id']."' method='POST'>
                            <input type='number' value='0' min='0' max='".$item['qtn']."' name='qtn' class='bnn1 n12'>
                            <input type='submit' value='Update Status' name='submit' class='btngreen'>
                        </form>
                    </td>
                </tr>";
            }
        ...
        else if ($fetch2['status'] == 'complete') {
            echo "<div class='text-white text-left'> <b>Order discount : </b> ".$fetch12['coupan discount']." % </div><br>"?>
        ...
        echo "<h1>No Record found ! Search Again</h1>";
    }
 
} else {
    echo "<h1>Search Please</h1>";
}
